using System.Diagnostics.CodeAnalysis;
using FrontDeskApp.Domain.Entities;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace FrontDeskApp.Infrastructure.Persistence.Configurations;

[ExcludeFromCodeCoverage]
internal sealed class StorageOrderConfiguration : IEntityTypeConfiguration<StorageOrder>
{
	public void Configure(
		EntityTypeBuilder<StorageOrder> builder)
	{
		builder.Property<Guid>("Id")
						.ValueGeneratedOnAdd()
						.HasColumnType("uniqueidentifier");

		builder.Property<DateTime?>("CreatedDateTime")
			.HasColumnType("datetime");

		builder.Property<Guid?>("CreatedId")
			.HasColumnType("uniqueidentifier");

		builder.Property<string>("CreatedName")
			.HasMaxLength(50)
			.HasColumnType("nvarchar(50)");

		builder.Property<Guid>("CustomerId")
			.HasColumnType("uniqueidentifier");

		builder.Property<byte>("IsSoftDeleted")
			.HasColumnType("tinyint");

		builder.Property<DateTime?>("RetrievedDate")
			.HasColumnType("datetime2");

		builder.Property<Guid>("StorageId")
			.HasColumnType("uniqueidentifier");

		builder.Property<DateTime>("StoredDate")
			.HasColumnType("datetime2");

		builder.Property<DateTime?>("UpdatedDateTime")
			.HasColumnType("datetime");

		builder.Property<Guid?>("UpdatedId")
			.HasColumnType("uniqueidentifier");

		builder.Property<string>("UpdatedName")
			.HasMaxLength(50)
			.HasColumnType("nvarchar(50)");

		builder.Property<byte[]>("Version")
			.IsConcurrencyToken()
			.ValueGeneratedOnAddOrUpdate()
			.HasMaxLength(8)
			.HasColumnType("timestamp");

		builder.HasKey("Id");

		builder.HasIndex("CustomerId");

		builder.HasIndex("IsSoftDeleted");

		builder.HasIndex("StorageId");

		builder.ToTable("StorageOrder");
	}
}
